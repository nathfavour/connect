              ...
              <Box sx={{ p: 2.5, opacity: isExpanded ? 1 : 0, transition: 'opacity 0.3s 0.1s', display: isExpanded ? 'block' : 'none' }}>
                <Stack spacing={2}>
                  ...
                  <Stack direction="row" spacing={1} justifyContent="flex-end">
                    <Button size="small" onClick={(e) => { _e.stopPropagation(); onDismiss(current.id); }} sx={{ color: 'rgba(255, 255, 255, 0.4)', textTransform: 'none', fontWeight: 700 }}>Later</Button>
                    {current.action && <Button variant="contained" size="small" onClick={(e) => { _e.stopPropagation(); current.action?.onClick(); onDismiss(current.id); }}
